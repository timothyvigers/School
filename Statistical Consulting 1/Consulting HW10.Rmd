...
So for 90% power to detect a difference in AZ of 0.6 mg/day, assuming known variation, Jamie will need 4 participants in each group (since the planned analysis requires 4 difference measurements). 

Check using an R function from methods:
```{r}
...
findPowerZ(diff = 0.6,sd = 0.3500204,n = 4)
findPowerZ(diff = 0.6,sd = 0.3500204,n = 3)
```
\pagebreak
...
With an unknown variance, R recommends a sample size of 9 participants per groups, which seems like a much better estimate to me (4 feels awfully low).
