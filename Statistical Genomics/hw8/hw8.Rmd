...
cells = estimateCellCounts(rgSet,meanPlot = T,compositeCellType = "Blood",cellTypes = c("CD8T","CD4T","NK","Bcell","Mono","Gran"),verbose = F)
...
m = as.data.frame(cbind(cells,t(m)))
```

```{r}
# Fit models in parallel
cl = makeCluster(8,type = "FORK")
l = parLapply(cl,colnames(m)[7:ncol(m)],function(c){
  #f = as.formula(paste0(c,"~",paste0(colnames(cells),collapse = "+")))
  f = as.formula(paste0(c,"~Sample_Group"))
  mod = lm(f,data = m)
  s = summary(mod)$coefficients
  return(c(s["Sample_Group","Estimate"],s["Sample_Group","Pr(>|t|)"]))
})
stopCluster(cl)
l = as.data.frame(do.call(rbind,l))
colnames(l) = c("Beta","P Value")
```

One thing I noticed when fitting these models is that many of the regression estimates for cell type are really large with correspondingly large standard errors. When I removed them from the model and re-ran the code I got the same results as `dmpFinder()` in part a), so I don't think it's a code issue. These estimates may not be a problem, but given more time I would probably investigate them a little. However, for this homework I am assuming that the estimates are okay given that we aren't interested in inference for cell type.
