...
```{r eval=FALSE}
dmp %>% filter(pval < 1e-5) %>% mutate(Direction = ifelse(sign(beta)==-1,"Hypomethylated","Hypermethylated")) %>% select(Probe,beta,Direction,pval) %>% rename("Beta"=beta,"P Value"=pval) %>% flextable(.) %>% footnote(.,i=1,j=3,value = as_paragraph("With respect to exposure"),ref_symbols = "*",part = "header") %>% set_table_properties(.,layout = "autofit",width = 0.75)
```
...
```{r}
data(yeastCDS)
allData = dataReading("./chipseq/tup1_IP.txt", "./chipseq/mock_IP.txt",
...
```{r}
# Find peaks
peak_location = peakLocation("./bPeaks_results.bed",yeastCDS$Saccharomyces.cerevisiae)
# Read results back in
promoters = read.delim("./bPeaksLocation_peakLocation_inPromoters.txt",header = F)
cds = read.delim("./bPeaksLocation_peakLocation_inCDS.txt",header = F)
```
