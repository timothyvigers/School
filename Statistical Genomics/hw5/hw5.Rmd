...
This method is similar to the `withinLaneNormalization()` function, but does not include a loess method. Instead of normalizing bins within a lane, however, these methods ensure that the lanes are similar. So, the median approach forces the median of all the lanes to the be the same, the upper approach does the same but using the upper quartile rather than the median, and the full method uses the full quantile method. First, generate each combination of within- and between-lane normalization methods:

# DO WE REALLY NEED A BEFORE AND AFTER PLOT FOR EACH COMBINATION PLUS A COMPARISON OF ALL 12?!
...
In order to more easily compare methods, it's helpful to look at the normalization boxplots in a grid. In this case the within-lane normalization approaches are in columns, and the between-lane approaches are in rows:

```{r,fig.height=10,fig.width=8}
par(mfrow=c(3,4))
boxplot(loess_median,main="Loess-Median")
boxplot(median_median,main="Median-Median")
boxplot(upper_median,main="Upper-Median")
boxplot(full_median,main="Full-Median")
boxplot(loess_upper,main="Loess-Upper")
boxplot(median_upper,main="Median-Upper")
boxplot(upper_upper,main="Upper-Upper")
boxplot(full_upper,main="Full-Upper")
boxplot(loess_full,main="Loess-Full")
boxplot(median_full,main="Median-Full")
boxplot(upper_full,main="Upper-Full")
boxplot(full_full,main="Full-Full")
```
