...
The `caret` package would pick $k=$ `r as.numeric(fit$bestTune)`, which was also selcted by my own CV code. So that's a good sign for both my code and the choice of $k$.
...
Based solely on this plot I think you could choose either $k=3$ or $k=4$ as the plot "elbow." However, we already know we have three different kinds of gene in this dataset, so I'll continue with that.
...
If you cut the dendrogram above at a high point (around 5 or 6), there are 3 groups which generally match the 3 clusters in the part above. However, using this method gene "YLR259C" is clustered with the two other heat shock proteins, which makes more intuitive sense than the kmeans results. 
