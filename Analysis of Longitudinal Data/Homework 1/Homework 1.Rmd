...
```{r}
mod1 <- gls(temp ~ year,data = temps,method = "ML",correlation = corAR1(form = ~1|fake))
```
...
It's pretty obvious that there is a pattern in these residuals, and I think the plot suggests that there could be a polynomial trend in the data.
...
plot(temps$year,new_resids,xlab = "Year",
     ylab = "Residuals",main = "New AR(1) Residuals")
```

Using the estimated correlation parameter from SAS to calculate new residuals looks much better. I think this model actually fits the data realtively well given these residuals, but it isn't perfect. The trend "W" trend from the previous plot it still sort of visible, although I doubt I'd notice it if I hadn't seen the first plot. 

Also, a qq plot of these residuals looks decent to me:

```{r echo=FALSE}
qqplot(temps$year,new_resids,ylab = "Residuals",main = "New AR(1) Residuals",xlab = "Year")
```
