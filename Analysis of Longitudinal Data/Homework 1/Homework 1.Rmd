...
This is easiest to solve with a given h, since the math works no matter what value you choose. Let's go with h = 2.

$$
E(\epsilon_t\epsilon_{t+2}) = E((Z_t + \phi Z_{t-1} +...)(Z_{t+2} + \phi Z_{t+1} + \phi^2 Z_{t} + \phi^3 Z_{t-1} + ...))\\
= E(Z_{t}Z_{t+2}+Z_{t}\phi Z_{t+1}+Z_{t}\phi^2 Z_{t}+\phi Z_{t-1}+\phi Z_{t-1}\phi^3 Z_{t-1}) 
$$

As long as the indices are different, the Z terms are independent. And because the expected value of each Z is 0, this simplifies to:

$$
E(\epsilon_t\epsilon_{t+1}) = E(\phi^2 Z_{t}^2 + \phi^4 Z_{t-1}^2+ \phi^6 Z_{t-2}^2+...) = \sum_{i=1}^\infty(\phi^2)^i Z_{t-i}^2
$$