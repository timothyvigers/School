...
minus_log_L_fun <- function(params) {
    ...
    log_L <- 
      sum(pcbs$Yi*(log(1-pnorm(log(0.001)-b0-b1*pcbs$Xi))/sigma)+
            (1-pcbs$Yi)*(log(pnorm(log(0.001)-b0-b1*pcbs$Xi))/sigma))    
    return (-log_L)
}
mle <- optim(par = c(1,1,1), minus_log_L_fun, method = "L-BFGS-B", lower = c(-7,-7,0), upper = c(100,100,100), hessian = TRUE)
```
...
```{r message=FALSE}
tobit <- AER::tobit(log(contam.lev) ~ factor(location), left = log(0.001), data = pcbs)
summary(tobit)
```
