...
pcbs$log_Yi <- ifelse(log(pcbs$contam.lev) < log(0.001),-10,log(pcbs$contam.lev))
# Tobit function
...
mle <- optim(par=init, tobit, y=pcbs$log_Yi, X=X, ll=-10,hessian = T)
mle$par
I <- mle$hessian
...
```{r message=FALSE}
tobit <- AER::tobit(log_Yi ~ factor(location), left = -10, data = pcbs)
summary(tobit)
```

## c.

Because the Tobit regression is on the log scale, the variance of the Probit estimators needs to be transformed before comparison.
