...
sqrt(var_Theta_probit/68)
```

This is a probit regression model, so we can use glm() to check the coefficient estimates:
...
mle <- optim(par=init, tobit, y=pcbs$log_Yi, X=X, ll=-7,hessian = T)
mle$par
I <- mle$hessian
...
```{r message=FALSE}
tobit <- AER::tobit(contam.lev ~ factor(location), left = 0.001, data = pcbs, dist = "lognormal")
summary(tobit)
```

## c.

Because the Tobit regression is on the log scale, the variance of the Probit estimators needs to be transformed before comparison. Based on this is appears that the Tobit estimators have greater asymptotic variance than the Probit ones. 
...
From part b, the average PCB level at Fort Erie is 

$$
\text{CI: }e^{\hat{\beta}_0\pm1.96*\frac{\sigma}{\sqrt{n}}}=e^{-5.26\pm1.96*\frac{0.97}{\sqrt{68}}}=e^{-5.26\pm0.23}=(e^{-5.49},\text{ }e^{-5.03})=(0.004,0.005)
$$
