...
What restriction allows it to be identified? Betas are the restricted parameters.

$$
log(Y_i^*) \text{~}N(\beta_0+\beta_1X_i,\sigma^2)\\
Yi = 1\text{ when }Y_i^* > l=0.001\\
Yi = 1\text{ when }log(Y_i^*) > log(l)\\
P(log(Y_i^*) > log(l)) = 1-P(log(Y_i^*)\leq log(l))\\
=1-P(\frac{log(Y_i^*)-\beta_0-\beta_1X_i}{\sigma}\leq\frac{log(l)-\beta_0-\beta_1X_i}{\sigma})=P(Z\leq\frac{log(l)-\beta_0-\beta_1X_i}{\sigma})\\
= 1 - \Phi(\frac{log(l)-\beta_0-\beta_1X_i}{\sigma})\\
\text{Set }P(log(Y_i^*) > log(l)) = \theta_i\\
Y_i\text{~ Bernoulli}(\theta_i)
$$

Calculate the log likelihood of Yi:

$$
L(Y|\theta)=\prod_{i = 1}^{n}\theta_i^{Y_i}(1-\theta_i)^{1-Y_i}\\
logL(Y|\theta)=\sum_{i = 1}^{n}Y_ilog(\theta_i)+(1-Y_i)log(1-\theta_i)\\
...
kable(summary(probit_mod)$coefficients)
```