...
```{r}
pcbs$log_Yi <- ifelse(log(pcbs$contam.lev) == -Inf,0,log(pcbs$contam.lev))
# Tobit function
...
initmod = lm(contam.lev ~ factor(location), data=pcbs)
X = model.matrix(initmod)
init = c(coef(initmod), log_sigma=log(summary(initmod)$sigma))

mle <- optim(par=init, tobit, y=pcbs$contam.lev, X=X, ll=log(0.001),hessian = T)
mle$par
I <- mle$hessian
```

This is a type I Tobit model, which can be fit using the VGAM package:
...
Because the Tobit regression is on the log scale, the standard error of the Probit estimators needs to be transformed before comparison. 

### i. Intercept (Location = Fort Erie)

```{r}
(0.28489)^2/(log(0.2366))^2
(exp(0.28489))^2/(0.2366)^2
```
